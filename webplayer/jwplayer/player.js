...
// HOVER PSEUDO-CLASS SUPPORT FOR IE6 
function initHovers() {
	$('.hoverable').hover(
		function() {
...
// TOP NAVIGATION 
function initTopNav() {
	var pathArray = ['players', 'addons', 'adsolution', 'bits-on-the-run'];
	var navIDArray = ['JWPlayerNav', 'AddOnsNav', 'AdSolutionNav', 'BotRNav'];
	var urlArray = window.location.pathname.split('/');
	
	for(i=0; i<= pathArray.length; i++) {
    	if(urlArray[1] == pathArray[i]) {
			var element = document.getElementById(navIDArray[i]);
			if(element) {
...
function lsHideResults(){
	gid("resultdiv").innerHTML = '.. or <a href="http://www.longtailvideo.com/sitemap" title="sitemap">check the sitemap</a>';
}
...
-------------------------------------------------------------------*/
function checkDL() {
	var cb = gid('licenseok');
	var ud = gid('updateme');
	var em = gid('email').value;
	if (cb.checked != true) {
		alert('Please read and agree to the license before downloading!');
		return false;
	} else {
		return true;
	}
...
function checkForm() {
	var tif = gid('titlefield');
	var tf = gid('textfield');
	var df = gid('dudefield');
	var pf = gid('parentfield');
	if (tif != null && tif.value.length < 5) {
		alert('Please insert a descriptive thread title.');
		return false;
	} else if(df != null && df.value.length < 2) {
		alert('Please insert your (nick)name.');
		return false;
	} else if(pf != null && pf.selectedIndex == 0) {
		alert('Please select a forum.');
		return false;
	} else if(tf != null && tf.value.length < 5) {
		alert('Your message is too short. Please add more information.');
		return false;
	} else {
...
function validateCaptcha()
{
    ...
    var html = $.ajax({
    type: "POST",
    url: "/support/forum/captcha",
    data: "recaptcha_challenge_field=" + challengeField + "&recaptcha_response_field=" + responseField,
    async: false
    }).responseText;
    if(html == "Entry posted successfully")
    {
        $("#captchaStatus").html("Entry posted successfully!");
        return true;
    }
    else
    {
...
function asyncSubscribe()
{
	if(ClickValidate())
	{
		$(".fadeelements").fadeOut(500);

        $.ajax({
		    type: "POST",
		    url: "/players/thankyou",
		    data: "download=1&email=" + $("#email").attr("value") + "&url=" + $("#url").attr("value") + "&sourcepage=" + $("#sourcepage").attr("value")
	    });

		$("#SubscriptionSuccess").fadeIn(500);
	}
}
